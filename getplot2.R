...
data <- data.table::fread(stringr::str_glue("{dir_work}1.data/wheatbelt经纬度and产量.csv")) %>%
  .[, yield := NULL]
head(data)
...
# 1.4 plot
rain <- data.table::fread(stringr::str_glue("{dir_work}wheatbelt{year_start}-{year_end}年平均累积降雨量.csv")) %>%
...
# cairo_pdf(file = stringr::str_glue("{dir_work}res/wheatbelt2000-2022年CWI_waterlogging_risk.pdf"), width = 34, height = 20, family = "Calibri", onefile = T)
ggplot() +
...
# 3 Generate and run the simulation file.------------------------------------------------------------------s
xmls <- dir(path = stringr::str_glue("{dir_work}1.data/soil/"), pattern = "xml$", full.names = T)

purrr::walk(.x = xmls, .f = function(xml){
  system(command = stringr::str_glue("sed -i 's|huhu|Soil|' {xml}"), intern = F)
}, .progress = T)

sites_done <- dir(path = stringr::str_glue("{dir_work}1.data/soil/"), pattern = "xml$", full.names = F)
sites_undone <- setdiff(paste0("site_", 1:568, ".xml"), sites_done)

xmls <- dir(path = stringr::str_glue("{dir_work}1.data/soil/"), pattern = "xml$", full.names = T)
purrr::walk(.x = xmls, .f = function(xml){
  row_end <- system(command = stringr::str_glue("grep -n 'Nutrients.Nutrient' {xml}"), intern = T) %>%
    stringr::str_split_i(string = ., pattern = ":", i = 1) %>%
    as.numeric() + 10
  
  system(command = stringr::str_glue("sed -i '{row_end}s|}|},|' {xml}"))
}, .progress = T)

# Generate running directories and files
config <- data.table::fread(stringr::str_glue("{dir_work}1.data/Australia_wheatbelt_lonlat.csv")) %>% 
  .[, State := stringr::str_squish(stringr::str_split_i(location, ",", -2L))] %>% 
  .[! State %in% c("Australian Capital Territory" , "Queensland", "Tasmania")]
head(config)
unique(config$State)
...
## Create a running root directory
dir_run_root <- stringr::str_glue("{dir_work}2.run")
if (! dir.exists(dir_run_root)) {
  dir.create(dir_run_root)
}

purrr::walk(.x = 1:nrow(config), .f = function(row, config, sites_2){ # 371有问题, 使用370的土壤
  single <- config[row]
  
  dir_run <- stringr::str_glue("{dir_run_root}/{single$site}")
  ...
  state <- single$State
  switch (state, 
          "Western Australia" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - WA.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations.apsimx"), overwrite = T),
          "South Australia" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - SA.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations.apsimx"), overwrite = T),
          "Victoria" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - VIC.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations.apsimx"), overwrite = T),
          "New South Wales" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - NSW.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations.apsimx"), overwrite = T))
  
  switch (state, 
          "Western Australia" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - WA_OC.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations_OC.apsimx"), overwrite = T),
          "South Australia" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - SA_OC.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations_OC.apsimx"), overwrite = T),
          "Victoria" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - VIC_OC.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations_OC.apsimx"), overwrite = T),
          "New South Wales" = file.copy(from = stringr::str_glue("{dir_work}1.data/WL_Rotations - NSW_OC.apsimx"), to = stringr::str_glue("{dir_run}/WL_Rotations_OC.apsimx"), overwrite = T))
  

  row_soil <- system(command = stringr::str_glue("grep -n '1961-01-01T00:00:00' '{dir_run}/WL_Rotations.apsimx'"), intern = T) %>%
    stringr::str_split_i(string = ., pattern = ":", i = 1) %>%
    as.numeric() %>% 
    .[1] + 7
  
  if (single$site == "site_371") {
    single$site <- "site_370"
  }
  switch (state, 
          "Western Australia" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations.apsimx'"), intern = F),
          "South Australia" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations.apsimx'"), intern = F),
          "Victoria" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations.apsimx'"), intern = F),
          "New South Wales" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations.apsimx'"), intern = F))
  
  switch (state, 
          "Western Australia" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations_OC.apsimx'"), intern = F),
          "South Australia" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations_OC.apsimx'"), intern = F),
          "Victoria" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations_OC.apsimx'"), intern = F),
          "New South Wales" = system(command = stringr::str_glue("sed -i '{row_soil}r {dir_work}1.data/soil/{single$site}.xml' '{dir_run}/WL_Rotations_OC.apsimx'"), intern = F))
  
  # Link met file
  system(command = stringr::str_glue("ln -s {dir_work}1.data/met/{single$site}.met {dir_run}/MET.met"))
  
}, config, sites_2, .progress = T)

# run model
sites <- dir(stringr::str_glue("{dir_work}2.run"), pattern = "site")

parallel::detectCores(logical = F)
future::plan("multisession", workers = 20)

res <- furrr::future_map_dfr(.x = sites, .f = function(x, dir_work){
  path_WL <- stringr::str_glue("{dir_work}2.run/{x}/WL_Rotations.apsimx")
  path_OC <- stringr::str_glue("{dir_work}2.run/{x}/WL_Rotations_OC.apsimx")
  
  OC_data <- try(apsimx::apsimx(file = basename(path_OC), src.dir = dirname(path_OC), silent = T, value = "CreateProfileTable", cleanup = T, simplify = F))
  
  WL_apsimx <- jsonlite::read_json(path = path_WL)
  
  for (i in 1:length(WL_apsimx[["Children"]][[1]][["Children"]][[12]][["Children"]][[4]][["Carbon"]])) {
    WL_apsimx[["Children"]][[1]][["Children"]][[12]][["Children"]][[4]][["Carbon"]][[i]] <- OC_data$TotCPerc[i]
    WL_apsimx[["Children"]][[1]][["Children"]][[12]][["Children"]][[4]][["SoilCNRatio"]][[i]] <- OC_data$CNRatio[i]
    WL_apsimx[["Children"]][[1]][["Children"]][[12]][["Children"]][[4]][["FBiom"]][[i]] <- OC_data$FBiom[i]
    WL_apsimx[["Children"]][[1]][["Children"]][[12]][["Children"]][[4]][["FInert"]][[i]] <- OC_data$FInert[i]
  }
  
  jsonlite::write_json(WL_apsimx, path = path_WL, pretty = TRUE, digits = NA, auto_unbox = TRUE, null = "null")
  Sys.sleep(5)
  
  # run
  res_data <- try(apsimx::apsimx(file = basename(path_WL), src.dir = dirname(path_WL), silent = T, value = "Report", cleanup = F, simplify = F)) %>%
    data.table::as.data.table()

  if (inherits(res_data, "try-error")) {
    res <- data.table(CheckpointID = NA, SimulationID = NA, FolderName = NA, Zone = NA, Clock.Today = NA, RotationManager.CurrentState = NA, WheatYield = NA, CanolaYield = NA, BarleyYield = NA, ChickpeaYield = NA, OatsYield = NA, Canola.FOXsum = NA, Wheat.FOXsum = NA, Barley.FOXsum = NA, Chickpea.FOXsum = NA, Oats.FOXsum = NA, OrganicC_30cm = NA, CO2_ton = NA, daily_denitn2o = NA, daily_nitn2o = NA, denitn2o_kg_ha_wheat = NA, nitn2o_kg_ha_wheat = NA, denitn2o_kg_ha_canola = NA, nitn2o_kg_ha_canola = NA, denitn2o_kg_ha_Barley = NA, nitn2o_kg_ha_Barley = NA, denitn2o_kg_ha_chickpea = NA, nitn2o_kg_ha_chickpea = NA, denitn2o_kg_ha_Oats = NA, nitn2o_kg_ha_Oats = NA, CO2_N2O_wheat = NA, CO2_N2O_canola = NA, CO2_N2O_Barley = NA, CO2_N2O_chickpea = NA, CO2_N2O_Oats = NA, daily_leachno3 = NA, no3n_in_leach_wheat = NA, no3n_in_leach_barley = NA, no3n_in_leach_canola = NA, no3n_in_leach_chickpea = NA, no3n_in_leach_Oats = NA, daily_runoffno3 = NA, no3n_in_runoff_wheat = NA, no3n_in_runoff_barley = NA, no3n_in_runoff_canola = NA, no3n_in_runoff_chickpea = NA, no3n_in_runoff_Oats = NA, daily_denitn = NA, denitn_kg_ha_wheat = NA, denitn_kg_ha_barley = NA, denitn_kg_ha_canola = NA, denitn_kg_ha_chickpea = NA, denitn_kg_ha_oats = NA, sumNO3Lost_kg_ha_wheat = NA, sumNO3Lost_kg_ha_barley = NA, sumNO3Lost_kg_ha_canola = NA, sumNO3Lost_kg_ha_chickpea = NA, sumNO3Lost_kg_ha_Oats = NA, daily_MineralisedN = NA, mineralisedn_kg_ha_wheat = NA, mineralisedn_kg_ha_barley = NA, mineralisedn_kg_ha_canola = NA, mineralisedn_kg_ha_chickpea = NA, mineralisedn_kg_ha_oats = NA, daily_NO3 = NA, tot_no3_kgha_wheat = NA, tot_no3_kgha_barley = NA, tot_no3_kgha_canola = NA, tot_no3_kgha_chickpea = NA, tot_no3_kgha_Oats = NA, daily_NH4 = NA, tot_nh4_kgha_wheat = NA, tot_nh4_kgha_barley = NA, tot_nh4_kgha_canola = NA, tot_nh4_kgha_chickpea = NA, tot_nh4_kgha_oats = NA, Date = NA, SimulationName = NA, site = x)
  } else {
    res <- res_data %>%
      setDT() %>%
      .[, site := x]
...
data.table::fwrite(res, stringr::str_glue("{dir_work}res/WL_Ratation_origin.csv"))
...
energy_data <- openxlsx::read.xlsx(stringr::str_glue("{dir_work}1.data/各种足迹/Energy footprint.xlsx"), sheet = 1) %>% 
  data.table::setDT()
energy_data
...
res_energy <- res[mean_energy_foot < 5000, .(mean_energy_foot = mean(mean_energy_foot)), by = .(State, SimulationName)]
res_energy
